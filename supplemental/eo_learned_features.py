...
# Code adapted from the w281 live session exercise
...
def get_embedding(in_imgs):
    ...
    for i in tqdm(range(in_imgs.shape[0])):
...
if __name__ == "__main__":
    # Run this py file directly if you want to generate the ResNet152 embeddings as ndarrays
    ...
    # Save these out since the whole featurization process can take around 20+ minutes
    np.save("train_embed.npy", train_learned_features)
    np.save("dev_embed.npy", dev_learned_features)
    np.save("test_embed.npy", test_learned_features)
